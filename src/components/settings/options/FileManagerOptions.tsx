...
export const FileManagerOptionsElement: React.FC<FileManagerOptionsElementProps> = ({ askBeforeOverwrite, onChange }) => {
    ...
    return <section>
        ...
        <CheckboxControl label="Uploading duplicate files"
                         description="Ask before overwriting duplicate files"
                         checked={askBeforeOverwrite}
                         onChange={(): void => onChange( "askBeforeOverwrite", !askBeforeOverwrite )} />
    </section>;
...
const FileManagerOptions: React.FC = observer( () => {
    const getValue = ( value: string ): boolean => settingsStore.getSetting( "filesystem", value );
    ...
    return <FileManagerOptionsElement askBeforeOverwrite={askBeforeOverwrite} onChange={onChange} />;
});
