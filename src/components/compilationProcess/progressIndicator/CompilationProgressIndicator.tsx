...
const stageDescriptions = [
    ...
    },
    {
        ...
        header: "Inform 7",
        description: "Turning the Inform 7 source text into intermediary Inform 6 code"
    },
    {
        stage: CompilationStage.secondPass,
        header: "Inform 6",
        description: "Compiling the intermediary Inform 6 code into a playable game file"
    },
    {
        stage: CompilationStage.results,
        header: "Results",
        description: "Collecting information about the generated files and resources"
    },
    {
...
export const CompilationProgressIndicatorElement: React.FC<CompilationProgressIndicatorElementProps> = ({ failed, percentage, stage }) => <Content>
    <ul className="steps is-vertical">
        {stageDescriptions.map( ( s ) => <CompilationProgressSegment key={s.stage}
                                                                     completed={s.stage < stage || stage === CompilationStage.finished}
                                                                     failed={failed}
...
const CompilationProgressIndicator: React.FC = observer( () => {
    ...
    return <CompilationProgressIndicatorElement failed={!success}
                                                percentage={percentage}
                                                stage={stage} />;
});
