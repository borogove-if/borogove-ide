...
export const FileItemElement: React.FC<FileItemElementProps> = ({
    file,
    isActive = false,
    isEntryFile,
    isFirst,
...
const FileItem: React.FC<FileItemProps> = observer(props => {
    ...
    const isEntryFile =
        !isSortable && projectStore.entryFile
            ? file.id === projectStore.entryFile.id
            : false;
    const isIncludePath = Boolean(
        file.type === MaterialsFileType.folder && file.isIncludePath
    );
    ...
    return (
        <FileItemElement
            ...
            isSortable={isSortable}
            onSort={(dir: 1 | -1): void =>
                materialsStore.moveCompilationOrder(file, dir)
            }
            {...props}
        />
    );
