...
export const FileActionsElement: React.FC<FileActionsElementProps> = ({
    ...
    return <div className="file-actions" onClick={stopPropagation} ref={ref}>
        {isSortable && <SortingIcons direction="up" enabled={!isFirst} onClick={sortUp} />}
        {isSortable && <SortingIcons direction="down" enabled={!isLast} onClick={sortDown} />}
        {isOpen && <div className="file-actions-overlay" onClick={(): void => onToggleOpen( false )} />}
        {!isReadOnly && <Dropdown isActive={isOpen} isAlign="right" className={isDropup ? "is-up" : ""}>
            <DropdownTrigger className="file-action-icon" onClick={(): void => onToggleOpen()}>
                <TiThMenu />
            </DropdownTrigger>
            <DropdownMenu>
                <DropdownContent>
 ...
 */
const FileActions: React.FC<FileActionsProps> = observer( ({ file, isEntryFile, isFirst, isLast, isLocked, isReadOnly, isSortable, onSort }) => {
    ...
    const showEntryFileActions = projectStore.manager && projectStore.manager.showFilesystemCompilerOptions && !projectStore.manager.orderedFiles && file.type !== MaterialsFileType.folder;
