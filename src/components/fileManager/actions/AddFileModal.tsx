...
export const AddFileModalElement: React.FC<AddFileModalElementProps> = observer( ({ folders, onConfirm, type }) => {
    ...
    return <ModalTemplate buttons={buttons} header={`New ${type}`}>
        <Field>
            <Label>
            ...
            <Control>
                <Input value={filename}
                       onChange={( e: React.FormEvent<HTMLInputElement> ): void => setFilename( ( e.target as HTMLInputElement ).value )}
                       autoFocus
                       required />
            </Control>
        </Field>
        {folders.length > 1 && <Field>
            <Label>
                Parent folder
            </Label>
            <Control>
                <Select onChange={( e: React.FormEvent<HTMLSelectElement> ): void => setSelectedPath( ( e.target as HTMLSelectElement ).value )}>
                    {folders.map( ( folder: string ) => <option key={folder} value={folder}>
                        {folder}
                    </option> )}
                </Select>
            </Control>
...
const AddFileModal: React.FC<AddFileModalProps> = observer( ({ type }) => {
    ...
    return <AddFileModalElement folders={folders}
                                onConfirm={onConfirm}
                                type={type} />;
});
