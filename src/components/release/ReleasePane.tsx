...
export const ReleasePaneElement: React.FC<ReleasePaneElementProps> = ({ cards }) => <Container>
    ...
    <Columns id="release-options-list">
        {cards.map( card => <Column key={card.id} isSize={card.wide ? "full" : undefined}>
            <ReleaseCard onBuild={card.onBuild} title={card.title} buttonText={card.buttonText}>
                {card.description}
            </ReleaseCard>
        </Column> )}
    </Columns>
    ...
    const cards = [
        {
            id: "publish",
            buttonText: "Publish",
            description: "Publish the game on Borogove.io. You'll get a link to play the game online. The game can optionally be published on the site's front page. You will be asked to log in with a Google account.",
            onBuild: (): void => { projectStore.release( ReleaseType.publish ); },
        ...
        },
        {
            ...
            onBuild: (): void => { projectStore.release( ReleaseType.gamefile ); },
        ...
        },
        {
            ...
            onBuild: (): void => { projectStore.release( ReleaseType.website ); },
